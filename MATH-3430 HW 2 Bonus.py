...
Q3: How will we get there?
A3: We will create an empty list, called result and will add the scaled 
vector using append. The scalar will be multiplied by the corresponding
element of the vector.
"""
"""

algorithm name: sca_vec_mult(scalar,vector)

Initializing result as an empty list, named result.
...
Q3: How will we get there?
A3: We will create an empty list, called result. Then we will replace the list 
with the output of sca_vec_mult and will add the scaled matrix using append to 
the result. The scalar will be multiplied by the corresponding vector of the 
matrix.

algorithm name: sca_mat_mult(scalar,matrix)

initialize result as an empty list, named result.

For each list in result, replace the list with the output of
sca_vec_mult(scalar,vector) and append product to result.
...
Q3: How will we get there?
A3: We will create en empty list, called result. Then for each list in result we will
replace it with add_vectors and append the sum of the corresponding vectors in 
...
from matrix_B) and append the sum to result.
...
Q2: What do we want?
A2: The matrix-vector multiplication of matrix and vector stored as a list.
...
initialize result as an empty list, same length as the first list in matrix, 
call it result.

for element in matrix append as 0 in result.

initialize vectors as an empty list, called lists.

for each list in matrix, overwrite it as the result from
sca_vec_mult(corresponding element of vector, list) and append to vectors.

for each list in matrix, replace result with add_vectors
...
def mat_vec_mult(matrix,vector):
    ...
    for list in lists:
        result = add_vectors(result,list)
...
Q2: What do we want?
A2: The product of matrix_A and matrix_B.
    
Q3: How will we get there?
A3: We will create an empty list, called result. Then by linear combination 
of the corresponding column used in mat_vec_mult we will then append the 
resultant matrix to result.
    
algorithm name: mat_mult

Initialize result as an empty list.

For column in matrix_B, replace the list with the output of mat_vec_mult and
